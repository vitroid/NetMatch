...
def polyhedra_iter(_cycles, maxnfaces=20, maxfragsize=0):
    ...
    def _IsDivided(fragment):
        "fragmentの頂点をとりのぞいたらグラフの成分が増えるなら、fragmentの内部にとりかこまれた頂点がある。それは十分単純なfragmentとは言えない。"
        ...
        logger.debug(
            "NCOMPO: {0} {1}".format(
                ...
                _ncompo))
        # この操作でグラフの成分が増えたら、fragmentの内部に無関係のノードが含まれていたと考える。
        compare = nx.number_connected_components(G2) > _ncompo
        if compare:
            logger.info([len(g) for g in nx.connected_components(G2)])
            logger.info([len(g) for g in nx.connected_components(_G)])
        return compare
